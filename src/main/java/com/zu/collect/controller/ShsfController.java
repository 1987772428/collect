    ...
    public String collect()
    {
        ...
        try {
            ...
            if (!html.equals("404")) {
                ...
                String regx = "(.*?)</div><divclass=lm2con6>(.*?)</div><divclass=lm2con7><pclass=p1><spanclass=num8>(.*?)</span></p><pclass=p1><spanclass=num8>(.*?)</span></p><pclass=p2><spanclass=num8>(.*?)</span>";
                String preDrawIssue;
                String preDrawCode;
                int id;
                int j = 0;
                while (m.find()) {
                    ...
                    if (n.find()) {
                        ...
                        preDrawCode = n.group(3).trim() + "," + n.group(4).trim() + "," + n.group(5).trim();
